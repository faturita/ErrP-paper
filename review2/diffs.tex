...
%DIF DEL ErrP.2.tex    Mon Oct 19 14:10:33 2020
%DIF ADD ../ErrP.tex   Wed Oct 21 08:38:47 2020
%% bare_jrnl.tex
%% V1.4b
%% 2015/08/26
%% by Michael Shell
...
\author{\DIFdelbegin \DIFdel{Bartolomé Francisco, Moreno Juan,  Navas Natalia, Vitali José, }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{Ramele~Rodrigo,~}%DIFDELCMD < \IEEEmembership{Member,~IEEE,}
%DIFDELCMD <         %%%
\DIFdel{Santos~Juan~Miguel%DIF <  <-this % stops a space
}\DIFdelend 
\DIFaddbegin \IEEEauthorblockN{%
...
\DIFaddend \thanks{R. Ramele and J.M.Santos are with the Department
...
% make the title area
\maketitle
\DIFaddbegin \begingroup\renewcommand\thefootnote{\textsection}
\footnotetext{\DIFadd{Equal contribution}}
\endgroup
\DIFaddend 
...
\begin{abstract}
\DIFdelbegin \DIFdel{In this study, we propose a simple game scenario that can be used to trigger a feedback response embedded in }\DIFdelend %DIF > objective
\DIFaddbegin \DIFadd{Error-related potential (ErrP) are a particular type of Event-Related Potential (ERP) elicited by a person attending a recognizable error.  These }\DIFaddend Electroencephalographic (EEG)  signals \DIFdelbegin \DIFdel{of an observation human critic that observes an agent playing a game.  Based on a }\DIFdelend \DIFaddbegin \DIFadd{can be used to train a gaming agent by a }\DIFaddend Reinforcement Learning (RL) \DIFdelbegin \DIFdel{model, the gaming agent receives rewards for their actions on the game and learns its }\DIFdelend \DIFaddbegin \DIFadd{algorithm to learn an }\DIFaddend optimal policy.  \DIFdelbegin \DIFdel{These rewards are obtained by implementing a Brain-Computer Interface (BCI) system that identifies signal components called Error-related Potentials (ErrP), which occur when a person witnesses a recognizable error.  We perform an experiment where rewards are obtained from the ErrP signals from observational human critics watching an agent playing randomly a game . The agent is iteratively trained by receiving these rewards and updating its policy, improving its overall performance}\DIFdelend \DIFaddbegin \DIFadd{The experimental process consists of an observational human critic (OHC) observing a simple game scenario while their brain signals are captured. The game consists of a grid, where a blue spot has to reach a desired target in the fewest amount of steps.  Results show that there is an effective transfer of information and that the agent successfully learns to solve the game efficiently, from the initial 97 steps on average required to reach the target to the optimal number of 8 steps}\DIFaddend .  Our results are expressed in threefold: (i) the \DIFdelbegin \DIFdel{structure }\DIFdelend \DIFaddbegin \DIFadd{mechanics }\DIFaddend of a simple grid-based game that can elicit the ErrP signal component\DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend (ii) the verification that \DIFdelbegin \DIFdel{low classification accuracy of just above chance level that produces noisy rewards is enough to allow an agent to learn the optimal policy}\DIFdelend \DIFaddbegin \DIFadd{the reward function only penalizes wrong steps, which means that type II error (not properly identifying a wrong movement) does not affect significantly the agent learning process}\DIFaddend ; (iii) collaborative rewards from multiple observational human critics can \DIFdelbegin \DIFdel{compensate the lack of accuracy or the }\DIFdelend \DIFaddbegin \DIFadd{be used to train the algorithm effectively and can compensate low classification accuracies and a }\DIFaddend limited scope of transfer learning schemes.
%DIF > Results
%DIF >  Expand the results of the paper as this is the contribution. “Results show that there is an effective transfer of information and that the agent learns successfully to solve the game eﬀiciently. Both the underlined terms need to cite evidence from the paper.
%DIF >  The rewards generated from different subjects can be used to train the same Q- Table to improve its performance, which may lead to strategies where the overall performance is improved based on the information from different human critics at the same time. //These are key findings, worth more discussion/interpretation and inclusion in abstract.
%DIF > The algorithm can be trained with information from different OHCs, since the resulting dataset is composed of rewards for each corresponding step, independent of who they were generated from. This shows that the error classification accuracy (approximately 0.67) is good enough for the algorithm to learn. The reward function only penalizes wrong steps, which means that type II error (not properly identifying a wrong movement) does not affect the accuracy, they only make the learning process slower.
%DIF > conclusions
%DIF > Our results are expressed in threefold (i) the structure of a simple grid-based game that can elicit the ErrP signal component; (ii) the verification that low classification accuracy of just above chance level that produces noisy rewards is enough to allow an agent to learn the optimal policy; (iii) collaborative rewards from multiple observational human critics can compensate the lack of accuracy or the limited scope of transfer learning schemes.
\DIFaddbegin 
...
Previous research has explored the usage of RL with reward signals based on brain activity, recorded by an EEG-based BCI system during task execution. The papers \cite{ROBOT-CONTROL-PAPER,Kim2017,Omedes2013} have successfully demonstrated that a robot can be controlled with brain signals from a person \DIFdelbegin \DIFdel{that }\DIFdelend \DIFaddbegin \DIFadd{who }\DIFaddend is observing a robot \DIFaddbegin \DIFadd{to }\DIFaddend solve a task.  Moreover, a growing number of studies have demonstrated the feasibility of using ErrPs as rewards for RL schemes such as to enhance robotic behaviour~\cite{Luo2019}, to assess air traffic controller's decisions~\cite{Goh2019} or to categorize actions as errors~\cite{Wirth2020}. Other approaches have used these signals as important feedback for human-robot interaction or to implement shared-control strategies~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Schiatti2018}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Schiatti2018,Iturrate2013}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .  Additionally, ErrPs have also been used in \DIFdelbegin \DIFdel{the context of }\DIFdelend games as an additional feedback channel that can be explored to improve gaming experience~\cite{Plass-OudeBos2010,kober2018bci}. 

Therefore,  we aim to use the information extracted from brainwaves to enhance the performance of a gaming agent.  The three contributions are (1) a simple game \DIFaddbegin \DIFadd{mechanics and agent }\DIFaddend that can elicit the ErrP potential, (2) results that confirm that even when ErrP classification accuracy is low \DIFdelbegin \DIFdel{and produces a noisy reward signal}\DIFdelend \DIFaddbegin \DIFadd{but with a high specificity}\DIFaddend , enough information is generated for an agent to learn the optimal policy and solve a simple game and (3) collaborative rewards from multiple observational human observers can compensate the lack of classification accuracy or the inefficacy of transfer learning procedures for brainwaves signals.

%DIF > However, even though this implies that the agent misses frequently that an action taken is wrong, this is not hindering the overall performance and the agent is still learning
\DIFaddbegin 
...
The retrieval of the OHC's brain activity, called the brainwave session, is one of the most critical parts of the study.  Subjects are recruited voluntarily and given a form with questions regarding their health (previous health issues and particular visual sensitivity), habits (sleeping hours, caffeine and alcohol consumption), and a \DIFaddbegin \DIFadd{written informed }\DIFaddend consent petition to collect the required data.  The \DIFaddbegin \DIFadd{experiment is conducted anonymously in accordance with the Declaration of Helsinki published by the World Health Organization. No monetary compensation is handed out. This study is approved by the Departamento de Investigación y Doctorado, Instituto Tecnológico de Buenos Aires (ITBA).  The }\DIFaddend brainwave sessions are performed with 8 subjects, 5 males and 3 females, with an average age of 25.12 years, a standard deviation of 1.54 years, and a range of 22-28 years. All subjects have normal vision, are right-handed and no history of neurological disorders.
...
This phase is divided into a sequence of run sessions and \DIFdelbegin \DIFdel{a }\DIFdelend gaming agent training \DIFdelbegin \DIFdel{match}\DIFdelend \DIFaddbegin \DIFadd{matches}\DIFaddend .  During the run session, the agent plays 200 matches guided by a specific Q-Table with a 5\% chance of randomly selecting a movement, to reduce deadlocks and loops.  Following the run session, the agent performs a single gaming agent training match.  The gaming agent starts first with a Q-Table initialized with zeros, so the initial policy for the agent is randomized.  For the agent to learn from the feedback generated by the OHC, movement actions are determined by the reply of the agent's actions that were taken during one brainwave session match, in an offline reinforcement learning scheme~\cite{Levine2020}. This allows the Q-Table to be \DIFdelbegin \DIFdel{built }\DIFdelend \DIFaddbegin \DIFadd{learned }\DIFaddend based on the OHC's feedback from the movements the agent took, which were executed pseudo-randomly during the brainwave session.  The previously mentioned feedback is not explicit as it comes from the interpreted brain signal data. This implies that the reward is determined by the OHC's brain activity.
...
\DIFdelbegin \DIFdel{Figure~\ref{fig:classifiers} }\DIFdelend \DIFaddbegin \DIFadd{Grand averaged time-locked signal segments of 2-seconds length for all the OHCs can be seen in Figure~\ref{fig:grandaverage}.  The ErrP can be noticed more clearly on parieto-central areas (Pz electrode), around 0.4~s with a more prominent positive peak.
}
...
\begin{figure}[ht]
    ...
    \caption{\DIFaddFL{Grand average of 2-seconds time-locked segments for all OHCs for the "move closer" (top) and "move further" (middle) condition. Zero corresponds to the onset of the stimulus, i.e. when the gaming agent moves. The increased height of the first peak around 0.4 seconds reflects the ErrP response particularly on Pz, based on the electrode layout (bottom).}}
    \label{fig:grandaverage}
\end{figure}


\DIFadd{Figure~\ref{fig:classifiers}, on the other hand, }\DIFaddend shows the binary classification accuracy obtained for the eight OHCs using five different classification algorithms and using a 10-fold cross-validation procedure.  The best overall performance is obtained using Logistic Regression.  
\DIFdelbegin \DIFdel{In addition, the classification accuracy obtained averaging 5 epochs is shown as well.  Although the signal averaging procedure improves the Signal-To-Noise-Radio (SNR) of the ErrP response, it reduces the number of data samples, producing a clear improvement only for the OHCs that contain more samples (1, 3 and 7).
}\DIFdelend 

\begin{figure}[ht]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.12]{revisedimages/cross_val.eps}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.5\textwidth,height=70pt]{revisedimages/cross_val.eps}
    %DIF > \includegraphics[width=290pt,height=90pt]{revisedimages/avg_5_segments.eps}
    \DIFaddendFL \caption{Binary single trial classification score using five different classifiers while recognizing ErrP potentials for the eight OHCs.   \DIFdelbeginFL \DIFdelFL{The classification score using an ensemble average of 5 epochs is shown as well (GrandAvg). }\DIFdelendFL Chance Level is 0.5.}
    \label{fig:classifiers}
\end{figure}


\DIFdelbegin \DIFdel{On the other hand}\DIFdelend \DIFaddbegin \DIFadd{Complementary}\DIFaddend , Figure \ref{fig:avg_steps} shows the average amount of steps it takes for the agent to reach the goal for each OHC, as the Q-Table is progressively trained using the reward information obtained from the prediction of the trained classifier.  Each point corresponds to a run session \DIFdelbegin \DIFdel{where }\DIFdelend \DIFaddbegin \DIFadd{of 200 game play repetitions.  The y-values represent }\DIFaddend the average number of steps \DIFdelbegin \DIFdel{for the agent }\DIFdelend \DIFaddbegin \DIFadd{the agent takes }\DIFaddend to reach the goal using a specific Q-Table \DIFdelbegin \DIFdel{is specified, for 200 repetitions}\DIFdelend \DIFaddbegin \DIFadd{for a run session}\DIFaddend . The first point, at the x-value 0, represents the number of steps the agent takes to reach the goal with an untrained Q-Table, where movements are decided randomly. The next point corresponds to the amount of steps it takes to reach the goal using a policy derived from a Q-Table trained after one brainwave session match, and so on.
...
In the case for OHC 5 and 6, the reward information obtained from the brainwaves is not enough to train the agent effectively. Figures \ref{fig:avg_steps} for OHC 5 and 6 show no apparent learning, as the amount of steps to reach the goal doesn't decrease when trained.  These results are also consistent with \DIFdelbegin \DIFdel{their }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend classification ROC curves, shown \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend Figures~\ref{fig:rocsubjects}\DIFdelbegin \DIFdel{obtained for both OHCs}\DIFdelend , where the area under the curve \DIFaddbegin \DIFadd{for OHCs 5 and 6 }\DIFaddend are close to chance level.  Both OHCs have less recorded data from the sessions in comparison to the rest of the OHCs.  This variation in performance for different OHCs has been studied extensively in BCI~\cite{Chavarriaga2014}.  Besides low data samples, there are other reasons affecting the classification accuracy:  cognitive reasons (i.e. the OHC not paying \DIFdelbegin \DIFdel{extensively }\DIFdelend \DIFaddbegin \DIFadd{extensive }\DIFaddend attention to the game dynamics), very low SNR of the ErrP component or even the BCI-illiteracy phenomena where the specific OHC's signals do not contain the expected component response~\cite{Yousefi2019}.
...
\end{subfigure}
\caption{ROC Curves for OHCs 1-8. True positive rate is on the vertical axis and false positive rate on the horizontal axis.  \DIFaddbeginFL \DIFaddFL{The ROC curves for ErrP identification for Subjects 5 and 6 show low classification scores.}\DIFaddendFL }
...
Electroencephalographic signals have high inter-subject variability~\cite{Chavarriaga2014}.  This is evidenced in Figure~\ref{fig:transferlearning} where the agent training is performed with rewards obtained by classifying epochs from one Tester OHC \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend a classifier that was trained using the brainwaves from a different Trainer OHC.  The figure shows the cumulative variation for all run sessions on the average number of steps required to reach the goal after training the agent with all the available matches from the brainwave session.  Enhancements are shown as negative values.  Only the diagonal of the heatmap matrix shows a clear improvement in terms of the reduction of the required number of steps to reach the goal (averaged per 200 runs) which corresponds to the same information for each OHC shown in Figure~\ref{fig:avg_steps}.  For \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend transfer learning experiment~\cite{Wu2016}, no performance gain is evidenced, the agents learn nothing and this implies that the reward information \DIFdelbegin \DIFdel{is useless}\DIFdelend \DIFaddbegin \DIFadd{provides no value}\DIFaddend .
...
\caption{Average steps using Q-Table trained with brainwave session matches from six different OHCs. X axis show the progressive number of gaming agent training matches used to train the Q-Table. \DIFaddbeginFL \DIFaddFL{These matches correspond to all subjects, excluding subjects 5 and 6 which individually show no significant learning progress.}\DIFaddendFL }
...
\DIFaddbegin \DIFadd{Each time the gaming agent plays this simple game, it takes on average around 100 steps to reach the target spot.  A classifier can be trained to recognize Error Potential from OHCs that observe the agent playing the game.  The gaming agent plays randomly and movements are marked based on the identification of an error potential from the OHC.  Those movements are used as rewards in a Reinforcement Learning scheme to train a Q-Table.  The gaming agent plays the game again, but this time the number of required steps to solve the game is reduced.  If rewards are provided based on random signals,  no reduction is achieved and the average number of steps does not change.   This shows that there is an effective transfer of information from the brainwaves to the agent.   As this process is repeated, the agent keeps improving solving the game effectively, i.e. performing the minimum number of required steps to reach the goal.
}


\DIFaddend %Enfatizar la idea de que la mejor métrica de la propia detección está dada por la propia capacidad del agente de minimizar su tarea, que hay un flujo de información que involucra la propia percepción de la persona.

This work aims to propose a simple game \DIFaddbegin \DIFadd{mechanics }\DIFaddend that can use the ErrP component to train a gaming agent using a RL model. \DIFaddbegin \DIFadd{As described in the literature~\mbox{%DIFAUXCMD
\cite{Chavarriaga2014}}\hspace{0pt}%DIFAUXCMD
, ErrPs can be used to transmit subjective feedback to a computer.  }\DIFaddend The collected data \DIFdelbegin \DIFdel{shows }\DIFdelend \DIFaddbegin \DIFadd{show }\DIFaddend that ErrP signals can in fact be classified and used to train an agent effectively.  \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend This proposal tries to keep the system as simple as possible, emphasizing information flow from the subjective error perception of the \DIFdelbegin \DIFdel{human critic.
Rewards are generated using the signal processing and classification pipeline, and the Q-Table updates, enhancing the performance of the gaming agent.
}\DIFdelend \DIFaddbegin \DIFadd{OHC.
}\DIFaddend 

One additional aspect to remark is the robustness of the learning strategy based on Q-Learning~\cite{Bauer2015,Rubin2012}.   The obtained accuracy to discriminate ErrPs is low\DIFdelbegin \DIFdel{.  However, even with such low accuracy  values}\DIFdelend \DIFaddbegin \DIFadd{, though on the same level to other similar results~\mbox{%DIFAUXCMD
\cite{Iturrate2013,Ehrlich2016}}\hspace{0pt}%DIFAUXCMD
.  In this regard, despite the variability of the ErrP response in terms of different experiments, levels of 90\%  accuracy in identifying it are reported~\mbox{%DIFAUXCMD
\cite{Chavarriaga2014}}\hspace{0pt}%DIFAUXCMD
.  Despite the low accuracy  in ErrP identification}\DIFaddend , the RL algorithm was able to extract meaningful information from rewards that were helpful to improve, and often maximize, the agent's performance.  Additionally,  \DIFdelbegin \DIFdel{one important aspect of the classification results is the }\DIFdelend \DIFaddbegin \DIFadd{classification results show a }\DIFaddend low percentage of false positives (Figure~\ref{fig:confusionmatrix}), \DIFdelbegin \DIFdel{showing }\DIFdelend \DIFaddbegin \DIFadd{entailing }\DIFaddend a high specificity. On the other hand, the percentage of false negatives is generally higher.  \DIFdelbegin \DIFdel{However, even }\DIFdelend \DIFaddbegin \DIFadd{Even }\DIFaddend though this implies that the agent misses frequently when a wrong action takes place, this is not hindering the overall performance and the agent is still learning. \DIFdelbegin \DIFdel{Though }\DIFdelend \DIFaddbegin \DIFadd{Although they may be }\DIFaddend scarce, accurate rewards are very useful for the RL algorithm.
%DIF < However, even though this imply that the agent misses frequently that an action taken is wrong, this do not seems to be a serious issue because the agent improves its performance.
...
At the same time, effective agent training depends on the OHC's training data. Results confirm the futility or complexity of using Transfer Learning~\cite{Wu2016}: training a classifier with data obtained from one OHC, and using the same classifier to identify ErrPs for another OHC does not \DIFdelbegin \DIFdel{increases }\DIFdelend \DIFaddbegin \DIFadd{increase }\DIFaddend the performance of the agent.  Despite that, the rewards generated from different subject's classifiers can be used to train the same Q-Table to improve its performance, which may lead to strategies where the overall performance is enhanced based on the information from different \DIFdelbegin \DIFdel{human critics }\DIFdelend \DIFaddbegin \DIFadd{OHCs }\DIFaddend at the same time.  \DIFdelbegin \DIFdel{There }\DIFdelend \DIFaddbegin \DIFadd{We hypothesize that as long as there are accurate rewards obtained from high specificity classifiers, there is extra information that can be used by the agent to get an improved Q-Table.  Additionally, it }\DIFaddend seems to be an agreement in terms of the subjective interpretation of what may be an appropriate movement to reach the goal\DIFaddbegin \DIFadd{, and different OHCs produce rewards for the same type of movements}\DIFaddend .
