...
In order to train the agent, a natural solution for this scenario comes handy from the field of Artificial Intelligence: Reinforcement Learning (RL)~\cite{Santos1999}. Recently, this technique has seen a come-back. Nonneglected is the influence of DeepBrain's AlphaGo project, which was the first to reach superhuman proficiency when it won the complex game Go against several world champions~\cite{ALPHA-GO}.
...
While using the previously mentioned step function, the Q-Table is updated in each iteration. This is done following the Equation \ref{equ:update_q_table}.  After the algorithm finishes iterating through all the training episodes, the Q-Table is saved. 
...
\end{subfigure}
\caption{Average number of steps for the agent when trained with experiences from subjects 1,2,3,4,5,6,7 and 8.}
\label{fig:avg_steps}
\end{figure}
...
It is a known fact in BCI that EEG signals have high inter-subject variability.  This is evidenced in Figure~\ref{fig:transferlearning} were the agent training is performed by using a classifier trained with experiences from one subject,  but tagging the experiences from a different subject using the predicted classification.  As the agent learns nothing, not performance gain is evidenced. 
...
\begin{subfigure}{0.25\textwidth}
    ...
    \caption{Average steps using Q-table trained with experiences from subject 8 classified with a classifier trained with data from subject 6.}
    \label{fig:subject5roc}
\end{subfigure}
\begin{subfigure}{0.25\textwidth}
    \centering
    \includegraphics[scale=0.2]{Images/Average_steps/alex_jose.png}
    \caption{Average steps using Q-table trained with experiences from subject 1 classified with a classifier trained with data from subject 3}
    \label{fig:subject6roc}
\end{subfigure}
...
\caption{Average steps using Q-Table trained with one experience per subject.}
