...
The simple setup of the game and board allows to further experimentation, using the reduction of the number of steps as a validation of the achieved information transfer.   It will be interesting to verify if the smooth progression towards the end alter the shape of the ErrP response.  REF

Further work will be conducted in order to increase the complexity of the game to allow the possibility that the target position be dynamically changed, or to consider how the ErrP response is triggered in relation with shapes and colors of the board markers~\cite{EIMER1997143}. In that case the agent would start to learn to follow the target, instead of learning to go to a specific static destination spot.  Additionally, the classifier could be enhanced to recognize more effectively the Error Potential~\cite{Iwane2017}.
