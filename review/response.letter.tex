...
\begin{quotation}
{\color{blue}
ErrP detection and experimentation is not an straightforward.  We tested many variants of the game and none could produce the ErrP.  We aimed to have a simple solution, a simple game.  The single subject accuracy that we show on Figure 6 are illustrative that the level of ErrP single trial identification that we achieved for different subjects (which is low).  We wanted to emphasize the finding that even with such low levels of identification (which is a common issue in BCI research \cite{REF}, it was possible for the agent to learn an efficient strategy.
}
\end{quotation}
...
\begin{quotation}
{\color{blue}
ESTE SERIA UN GRAFICO EN UNA TABLA DE 8X8 DONDE SE MUESTRAN TODAS LAS COMBINACIONES POSIBLES. CREEN QUE ESTO SERIA ALGO FACTIBLE ?
}
\end{quotation}
...
\begin{quotation}
{\color{blue}
TIENE RAZON.  ACA PODEMOS MOSTRAR TODAS LAS CURVAS RESUMIDAS.  EL TEMA ES QUE HAY QUE CAMBIAR LOS LABELS PARA QUE PUEDAN AJUSTARSE COMO SI FUESEN GRAFICOS CHICOS, QUEDARIA DE 4X4.
}
\end{quotation}
...
\begin{quotation}
{\color{blue}
HAY QUE MOSTRAR TODAS LAS MATRICES DE CONFUSION.
}
\end{quotation}
